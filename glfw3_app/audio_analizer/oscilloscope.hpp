	...
	*/
	...
	public:
		...
		*/
		...
		public:
			info_param() noexcept :
				grid_color_(img::rgba8(255, 255, 255, 96)), grid_step_(30),
	...
	private:
		...
		double		sample_rate_;	///< サンプリング・レート
		double		time_grid_;		///< 1 grid 辺りの時間[sec]
	...
	public:
		...
		*/
		...
		//-----------------------------------------------------------------//
		/*!
			@brief  サンプリングレートの設定（周波数）
			@param[in]	t	周波数
		*/
		//-----------------------------------------------------------------//
		void set_sample_rate(double t) noexcept { sample_rate_ = t; }


		//-----------------------------------------------------------------//
		/*!
			@brief  タイムグリッドの設定（１グリッド辺りの時間）
			@param[in]	t	時間
		*/
		//-----------------------------------------------------------------//
		void set_time_grid(double t) noexcept { time_grid_ = t; }
		...
		*/
		...
		/*!
			...
			@param[in]	frq		生成周波数 [Hz]
			@param[in]	gain	ゲイン（1.0 で、最大振幅）
		*/
		//-----------------------------------------------------------------//
		void build_sin(uint32_t ch, double smp, double frq, double gain) noexcept
		{
			if(ch >= CHN) return;
		...
		*/
		//-----------------------------------------------------------------//
		void render(const vtx::ipos& size) noexcept
		{
			double dt = 1.0 / sample_rate_;
			uint32_t tstep = time_grid_ / static_cast<double>(info_.grid_step_) / dt * 65536.0;
			...
			for(uint32_t n = 0; n < CHN; ++n) {
				...
				if(t.units_.empty()) update = false;
					...
					if(t.param_.volt_scale_ != volt_scale_[n]) {
						volt_scale_[n] = t.param_.volt_scale_;
						update = true;
					}
					if(t.param_.volt_grid_ != volt_grid_[n]) {
						volt_grid_[n] = t.param_.volt_grid_;
						update = true;
					}
