...
namespace img {
	...
	*/
	...
	public:
		...
		*/
		//-----------------------------------------------------------------//
		jpeg_io() : imf_(0), quality_(75), rgb_ptr_(0) { }
		...
		*/
		//-----------------------------------------------------------------//
		const char* get_file_ext() const { return "jpg,jpeg"; }
		...
		*/
		...
		/*!
			@brief	RGB イメージの登録@n
					※RGB の並びでパックされたアレイを渡す事により、最小限@n
					のコピーと参照でより高速に処理できる。
			@param[in]	ptr	RAW イメージのポインター
			@param[in]	w	RAW イメージ横幅
			@param[in]	h	RAW イメージ高さ
			@param[in]	vf	縦を反転する場合「true」
		*/
		//-----------------------------------------------------------------//
		void set_image_RGB(const unsigned char* ptr, int w, int h, bool vf = true) {
			rgb_ptr_ = ptr; rgb_w_ = w; rgb_h_ = h; rgb_vf_ = vf;
		}
