...
namespace vtx {
	...
	template <typename T>
	constexpr T get_pi() { return static_cast<T>(3.1415926535897932384626433832795); }

	constexpr float  deg2rad_f_ = get_pi<float>() / 180.0f;		///< DEG -> RAD の変換定数(float)
	constexpr float  radian_f_  = 2.0f * get_pi<float>();		///< RADIAN のπ変換定数(float)
	constexpr double deg2rad_d_ = get_pi<double>() / 180.0;		///< DEG -> RAD の変換定数(double)
	constexpr double radian_d_  = 2.0 * get_pi<double>();		///< RADIAN のπ変換定数(double)
	...
	*/
	...
	inline fvtx get_vtx(const fvtxs& src, int pos)
	{
	...
	*/
	//-----------------------------------------------------------------//
	static bool clumped_bspline(const fvtxs& src, int lod, fvtxs& dst)
	{
...
//				float b3 =         it * it * it;
	...
	*/
	//-----------------------------------------------------------------//
	static bool division_lines(const fvtxs& src, int lod, fvtxs& dst)
	{
		int size = (int)src.size();
	...
	*/
	//-----------------------------------------------------------------//
	static bool intersection_line(const fpos_line& a, const fpos_line& b, fpos& pos)
	{
		fpos ad = a.t - a.s;
		fpos bd = b.t - b.s;
	...
	*/
	//-----------------------------------------------------------------//
	static bool intersection_line(const dpos_line& a, const dpos_line& b, dpos& pos)
	{
		dpos ad = a.t - a.s;
		dpos bd = b.t - b.s;
	...
	*/
	//-----------------------------------------------------------------//
	static void set_min(const fvtx& src, fvtx& min)
	{
	...
	*/
	//-----------------------------------------------------------------//
	static void set_max(const fvtx& src, fvtx& max)
	{
	...
	*/
	//-----------------------------------------------------------------//
	static void set_min_max(const fvtx& src, fvtx& min, fvtx& max)
	{
		if(min.x > src.x) min.x = src.x;
	...
	*/
	//-----------------------------------------------------------------//
	static void scan_min_max(const fposs& src, fpos& min, fpos& max)
	{
		bool init = false;
	...
	*/
	//-----------------------------------------------------------------//
	static void scan_min_max(const dposs& src, dpos& min, dpos& max)
	{
		bool init = false;
	...
	*/
	//-----------------------------------------------------------------//
	static void scan_min_max(const fvtxs& src, fvtx& min, fvtx& max)
	{
		bool init = false;
	...
	*/
	//-----------------------------------------------------------------//
	static void scan_min_max(const dvtxs& src, dvtx& min, dvtx& max)
	{
		bool init = false;
	...
	*/
	//-----------------------------------------------------------------//
	static bool make_share_vertex(const fvtxs& src_a, const fvtxs& src_b, fvtxs& dst, bool share = true)
	{
