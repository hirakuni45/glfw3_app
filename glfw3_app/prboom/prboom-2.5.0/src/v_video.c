...
void V_UpdateTrueColorPalette(video_mode_t mode) {
  ...
  if (mode == VID_MODE32) {
    if (!Palettes32) {
      // set int palette
      Palettes32 = (unsigned int*)malloc(numPals*256*sizeof(int)*VID_NUMCOLORWEIGHTS);
      for (p=0; p<numPals; p++) {
        for (i=0; i<256; i++) {
    ...
    if (!Palettes16) {
      // set short palette
      Palettes16 = (unsigned short*)malloc(numPals*256*sizeof(short)*VID_NUMCOLORWEIGHTS);
      for (p=0; p<numPals; p++) {
        for (i=0; i<256; i++) {
          r = gtable[pal[(256*p+i)*3+0]];
          g = gtable[pal[(256*p+i)*3+1]];
    ...
    if (!Palettes15) {
      // set short palette
      Palettes15 = (unsigned short*)malloc(numPals*256*sizeof(short)*VID_NUMCOLORWEIGHTS);
      for (p=0; p<numPals; p++) {
        for (i=0; i<256; i++) {
          r = gtable[pal[(256*p+i)*3+0]];
          g = gtable[pal[(256*p+i)*3+1]];
