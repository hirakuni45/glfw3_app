...
TGAWrite(TGA 	     *tga, 
	 ...
	 size_t       size)
{
	size_t wrote = tga->file.write(buf, size, tga->fd);
...
TGAWriteHeader(TGA *tga)
{
	...
	if (TGAWrite(tga, tmp, TGA_HEADER_SIZE) != TGA_HEADER_SIZE) {
		free(tmp);
		...
		const tbyte *buf)
{
	...
	if (TGAWrite(tga, buf, tga->hdr.id_len) != tga->hdr.id_len) {
		TGA_ERROR(tga, TGA_WRITE_FAIL);
		return TGA_ERROR;
	}
	    ...
	    tbyte *buf)
{
	...
	for (x = 1; x < width; ++x) {
		if (memcmp(buf, buf + bytes, bytes)) {
			if (repeat) {
				tga->file.putc(128 + repeat, tga->fd);
				tga->file.write(from, bytes, tga->fd);
				from = buf + bytes; 
				repeat = 0;
				direct = 0;
			} else { 
			...
			if (direct) {
				tga->file.putc(direct - 1, tga->fd);
				tga->file.write(from, bytes * direct, tga->fd);
				from = buf; 
				direct = 0;
				repeat = 1;
			} else {
		...
		if (repeat == 128) {
			tga->file.putc(255, tga->fd);
			tga->file.write(from, bytes, tga->fd);
			from = buf + bytes;
			direct = 0;
			repeat = 0;
		} else if (direct == 128) {
			tga->file.putc(127, tga->fd);
			tga->file.write(from, bytes * direct, tga->fd);
			from = buf + bytes;
			direct = 0;
			repeat = 0;
	...
	if (repeat > 0) {
		...
		tga->file.putc(direct, tga->fd);
		tga->file.write(from, bytes * (direct + 1), tga->fd);
	}
