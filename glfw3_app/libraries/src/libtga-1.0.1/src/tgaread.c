...
TGARead(TGA    *tga, 
	...
	size_t 	size)
{
	size_t read = tga->file.read(buf, size, tga->fd);
...
TGAReadHeader (TGA *tga)
{
	...
	if (tga->file.read(tmp, TGA_HEADER_SIZE, tga->fd) != TGA_HEADER_SIZE) {
		free(tmp);
...
TGAReadImageId(TGA    *tga, 
	       tbyte **buf)
{
	if (!tga || tga->hdr.id_len == 0) return TGA_ERROR;
	...
	*buf = (tbyte*)malloc(tga->hdr.id_len);
	...
	if (TGARead(tga, *buf, tga->hdr.id_len) != tga->hdr.id_len) {
		free(buf);
		TGA_ERROR(tga, TGA_READ_FAIL);
		return TGA_ERROR;
	}
	   ...
	   tbyte *buf)
{
	...
	for (x = 0; x < width; ++x) {
		if (repeat == 0 && direct == 0) {
			head = tga->file.getc(tga->fd);
			if (head == EOF) return TGA_ERROR;
			if (head >= 128) {
				repeat = head - 127;
				if (tga->file.read(sample, bytes, tga->fd) != bytes) 
					return TGA_ERROR;
			} else {
				direct = head + 1;
			}
		...
		if (repeat > 0) {
			...
			if (tga->file.read(buf, bytes, tga->fd) != bytes) return TGA_ERROR;
			--direct;
		}
		buf += bytes;
	}
