...
namespace app {
	...
	*/
	...
	struct unit_base {
		...
		*/
		...
		//	リットル	1	l
		//	キロリットル	0.001	kl
		//	立方ミリメートル	1,000,000	mm3
		//	立方センチメートル	1,000	cm3 (cc)
		//	立方メートル	0.001	m3
		//ヤード・ポンド法:
		//	立方インチ	61.023744094732	cu in
		//	立方フィート	0.035314666721489	cu ft
		//	エーカー・フィート	8.1071319378991E-7	ac ft
		//英:
		//	液量オンス(英)	35.195079727854	fl oz
		//	ジル(英)	7.0390159455708	gi
		//	パイント(英)	1.7597539863927	pt
		//	クォート(英)	0.87987699319635	qt
		//	ガロン(英)	0.21996924829909	gal
		//	バレル(英)	0.0061102568971969	bl
		//米液量:
		//	液量オンス(米)	33.814022701843	fl oz
		//	ジル(米)	8.4535056754608	gi
		//	パイント(米液量)	2.1133764188652	pt
		//	クォート(米液量)	1.0566882094326	qt
		//	ガロン(米液量)	0.26417205235815	gal
		//	バレル(米液量)	0.0083864143605761	bl
		//米乾量:
		//	パイント(米乾量)	1.8161659685377	pt
		//	クォート(米乾量)	0.90808298426886	qt
		//	ガロン(米乾量)	0.22702074606721	gal
		//	ペック(米乾量)	0.11351037303361	pk
		//	ブッシェル(米乾量)	0.028377593258402	bu
		//	バレル(米乾量)	0.0086484093739891	bl
		//石油:
		//	バレル(石油)	0.0062898107704321	bbl
		//尺貫法:
		//	勺(シャク)	55.435235318617	勺
		//	合(ゴウ)	5.5435235318617	合
		//	升(ショウ)	0.55435235318617	升
		//	斗(ト)	0.055435235318617	斗
		//	石(コク)	0.0055435235318617	石
		static constexpr char volume_tbl[] = {
			"1000\0ml\0"
			"1\0l\0"
			"0.1\0kl\0"
			"61.023744094732\0cu in\0"
			"35.195079727854\0oz e\0"
			"33.814022701843\0oz u\0"
			"0.21996924829909\0gal e\0"
			"0.26417205235815\0gal u\0"
			"0.0062898107704321\0bbl\0"
			"0.55435235318617\0shyo\0"
			"\0"
		};
	...
	*/
	...
	class unit : public unit_base {
		...
		static constexpr uint32_t VOLUME_N = static_cast<uint32_t>(VOLUME::_);
		static constexpr uint32_t VOLUME_O = WEIGHT_O  + WEIGHT_N;
		static constexpr uint32_t MONEY_N  = static_cast<uint32_t>(MONEY::_);
		static constexpr uint32_t MONEY_O  = VOLUME_O  + VOLUME_N;
		unit_t	units_[LENGTH_N + SPEED_N + WEIGHT_N + VOLUME_N + MONEY_N];
	...
	public:
		...
		*/
			...
			type_(TYPE::NIL)
		{
			...
			out_[4].volume = inp_[4].volume = VOLUME::l;
			out_[5].money  = inp_[5].money  = MONEY::YEN;
			...
			make_const_value_(unit_base::volume_tbl, VOLUME_O, VOLUME_N);
		...
		*/
		...
		bool set_rate(MONEY type, const char* rate) noexcept
		{
			...
			tmp[l] = 0;
			if(ret) {
				strcpy(&money_[MONEY_BFN * static_cast<uint32_t>(type)], tmp);
			}
			return ret;
		}
