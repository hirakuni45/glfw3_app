...
short  negonearray[SCREENWIDTH] = { 0 };
short  screenheightarray[SCREENWIDTH] = { 0 };
...
{ 
    ...
    for (i = 0; i < numsprites; i++) {
		...
		for (l = firstspritelump; l < (lastspritelump + 1); l++) {
			const char* name = lumpinfo[l].name;
			if(strncmp(name, namelist[i], 4) == 0) {
				frame = name[4] - 'A';
				rotation = name[5] - '0';
...
/// printf("InitSprite: '%s' %d/%d, %s, patched: %d, frame: %d, rotation: %d\n",
///	lumpinfo[l].name, i, numsprites, spritename, patched, frame, rotation);
