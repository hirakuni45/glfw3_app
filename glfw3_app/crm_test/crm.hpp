...
///			auto md = get_module(swn);
			for(int i = 0; i < num; ++i) {
				...
				gui::widget_check::param wp_(std::format("{}", swn));
		...
		*/
		...
		void init(gui::widget* root, int d_w, int ofsx, int ofsy)
		{
			...
			{  // exec
				...
				exec_->at_local_param().select_func_ = [=, this](int n) {
					auto s = make_crm_param_();
					serial_.write(s.c_str(), s.size());
					serial_out_ = true;
					termcore_->output(std::format("Start Measure: {}\n", n));
					termcore_->output(s);
					ans_->set_text("");
				};
			}
			...
			{  // キャリブレーション・ボタン
				...
				carib_->at_local_param().select_func_ = [=, this](int id) {
					...
					serial_.write(s.c_str(), s.size());
					serial_out_ = true;
					termcore_->output(std::format("Start carib: {}\n", n));
					termcore_->output(s);
		...
		void update()
		{
			...
			if(crm_line_.size() >= 9 && crm_line_[8] == '\n') {
				...
				if(key == "CRCD") {
					...
					termcore_->output(std::format("CRCD: {:08X}\n", crcd_value_));
					crcd = true;
				} else if(key == "CRRD") {
					crrd_value_ = get32_(&crm_line_[4]);
					termcore_->output(std::format("CRRD: {:08X}\n", crrd_value_));
					crrd = true;
				} else if(key == "CRDD") {
					crdd_value_ = get32_(&crm_line_[4]);
					termcore_->output(std::format("CRDD: {:08X}\n", crdd_value_));
					crdd = true;
				}
			...
			if(carib_task_ != carib_task::idle) {
				if(crcd) {
					...
					std::string s = std::format("{:6.5f}", (SIP / CRP));
			...
			if(mode_->get_select_pos() == 0 && crrd) {  // CRRD
				...
				a /= itbl[amps_->get_select_pos()];
				ans_->set_text(std::format("{:6.5f} Ω", a));

			} else if(mode_->get_select_pos() != 0 && crcd) {  // 容量、合成容量の場合

				double SRP = static_cast<double>(crrd_value_ - crdd_value_);
				termcore_->output(std::format("SRP: {:6.5f}\n", SRP));
				double SIP = static_cast<double>(crcd_value_ - crdd_value_);
				termcore_->output(std::format("SIP: {:6.5f}\n", SIP));
...
//        	CIP = SIP - A_CAL * SRP  //(SIP/SRP)の値（位相補正係数）を使用します。（後述）
				...
				termcore_->output(std::format("CRP: {:6.5f}\n", CRP));
				double CIP = SIP - (A_CAL * SRP);
				termcore_->output(std::format("CIP: {:6.5f}\n", CIP));
//			CORは抵抗値計算用の定数係数 = 4.6588E-8 @n
//			COCは容量値計算用の定数係数 = 21454732.4 @n
//			_iはユーザーが選択した測定電流値 [Ap-p] @n
//			_fはユーザーが選択した測定周波数 [Hz] @n
...
//			ここで：@n
				...
				termcore_->output(std::format("R: {:6.5f} [ohm]\n", R));
				double PAI = 3.141592654;
				double C = (COC * _i * CIP) / ((2 * PAI * _f) * ((CRP * CRP) + (CIP * CIP))); 
				termcore_->output(std::format("C: {:6.5f} [uF]\n", (C * 1e6)));

				if(mode_->get_select_pos() == 1) {
					ans_->set_text(std::format("{:6.5f} uF", C));
				} else {
					ans_->set_text(std::format("{:6.5f} ohm / {:6.5f} uF", R, C));
				}
			}
		}
