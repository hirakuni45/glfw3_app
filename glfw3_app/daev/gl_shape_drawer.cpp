...
void gl_shape_drawer::drawCoordSystem()  {
	glBegin(GL_LINES);
	glColor3f(1, 0, 0);
	glVertex3d(0, 0, 0);
	glVertex3d(1, 0, 0);
...
void gl_shape_drawer::drawSphere(btScalar radius, int lats, int longs) 
{
	int i, j;
	for(i = 0; i <= lats; i++) {
...
void gl_shape_drawer::drawCylinder(float radius,float halfHeight, int upAxis)
{
...
gl_shape_drawer::ShapeCache*		gl_shape_drawer::cache(btConvexShape* shape)
{
...
void gl_shape_drawer::drawOpenGL(btScalar* m, const btCollisionShape* shape, const btVector3& color,int	debugMode,const btVector3& worldBoundsMin,const btVector3& worldBoundsMax)
{
...
//
void		gl_shape_drawer::drawShadow(btScalar* m,const btVector3& extrusion,const btCollisionShape* shape,const btVector3& worldBoundsMin,const btVector3& worldBoundsMax)
{
	glPushMatrix(); 
///	btglMultMatrix(m);
	glMultMatrixf(m);
